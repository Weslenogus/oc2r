...
### Lua computers
Alongside the RISC-V computer there is a second machine, the **Lua Computer**, which runs the OpenComputers 1 Lua environment on real Lua 5.3 - the environment software such as OpenOS and MineOS is written against. Place one with a **Lua Screen** against it, the way an OpenComputers case takes a monitor, and right click the screen: it needs no power and no parts, it carries its own disk, and a computer with an empty disk boots the shell in its ROM, so there is a prompt to type at from the start. Right clicking the case opens its own panel - power, run state, and how many screens it can see. The walkthrough is [here][lua documentation], and in the in-game manual.
